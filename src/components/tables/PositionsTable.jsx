 ...
 *
 * On the phone card the state badge is the headline, since "is anything
 * jammed" is the whole reason to open this screen on a phone in the first
 * place — and a stuck row keeps its warning tint there too.
 */
    ...
    <>
      ...
      <CardList>
        {positions.map((p) => {
          ...
          const stuck = p.state !== 'FLAT' && heldMs > STALE_AFTER_MS;
          return (
            <RecordCard key={p.symbol} warn={stuck}>
              <RecordHead
                lead={<span className="text-ink truncate text-sm font-semibold">{p.symbol}</span>}
                trail={<StateBadge state={p.state} />}
              />

              <RecordMeta>
                <span>Since {formatTime(p.updated_at)}</span>
              </RecordMeta>
